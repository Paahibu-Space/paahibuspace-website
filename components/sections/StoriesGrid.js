  ...
  return (
    <section className="py-16 bg-background-light">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Filter Tabs */}
        <div className="flex overflow-x-auto pb-4 mb-10 gap-2 border-b border-gray-100 hide-scrollbar">
          <Button variant="secondary" size="sm" className="rounded-full">
            All Stories
          </Button>
          <Button variant="ghost" size="sm" className="rounded-full text-secondary hover:bg-gray-50">
            Impact
          </Button>
          <Button variant="ghost" size="sm" className="rounded-full text-secondary hover:bg-gray-50">
            Mentorship
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {stories.map((story, index) => (
            <article
              key={index}
              className="group flex flex-col bg-white rounded-xl border border-gray-100 overflow-hidden shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300"
            >
              <div className="relative aspect-video overflow-hidden bg-gray-100">
                ...
                <div className="absolute top-4 left-4 z-20">
                  <span className="px-3 py-1 bg-white/90 backdrop-blur text-xs font-bold text-secondary rounded-full uppercase tracking-wider">
                    {story.tag}
                  </span>
                </div>
              </div>
              <div className="flex flex-col flex-grow p-6">
                ...
                <div className="mt-auto">
                  <Link
                    href="#"
                    className="inline-flex items-center text-primary font-semibold text-sm hover:gap-2 transition-all group-hover/link"
                  >
                    Read Story{" "}
